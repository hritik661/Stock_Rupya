...
export async function POST(req: Request) {
  try {
    ...
    // Revert the top gainer payment - also clear predictions access so revert in one module clears both
    if (useDatabase && sql) {
      try {
        // Clear both payment flags on the user (top_gainers and predictions)
        await sql`UPDATE users SET is_top_gainer_paid = false, is_prediction_paid = false WHERE id = ${user.id}`
        ...
        try {
          ...
          return NextResponse.json({
            success: true,
            message: 'Payment access has been reverted for top gainers and predictions.',
            user: {
              id: u?.id || user.id,
          ...
          return NextResponse.json({
            success: true,
            message: 'Payment access reverted (partial).',
            user: { id: user.id, email: user.email, isTopGainerPaid: false, isPredictionPaid: false }
          })
        }
      } catch (err) {
